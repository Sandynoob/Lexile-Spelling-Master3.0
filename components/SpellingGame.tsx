            ...
            <div className="flex flex-wrap justify-center gap-1.5 md:gap-3 max-w-full">
              {selectedSegments.map((segment, i) => (
                <div 
                   ...
                   `}
                   style={{
                     minWidth: `clamp(2.4rem, ${65 / segmentCount}vw, ${segmentCount > 5 ? '3.2rem' : '4.8rem'})`,
                     height: `clamp(3.2rem, 10vh, ${segmentCount > 5 ? '4rem' : '5.8rem'})`,
                     fontSize: `clamp(1.1rem, ${32 / segmentCount}vw, 2rem)`
                   }}
                >
                  {segment || ''}
                </div>
          ...
          <div className="flex-none flex flex-wrap justify-center gap-2.5 md:gap-5 max-w-full px-4">
            {scrambledSegments.map((segment, i) => (
              <button
                ...
                style={{
                  minWidth: `clamp(3.5rem, 14vw, 5.5rem)`,
